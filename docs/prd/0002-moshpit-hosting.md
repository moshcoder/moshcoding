...
- R7 [P1] **Reverse Proxy / Ingress**: The hosting layer exposes a reverse proxy so `scrambled.eggs:80` and `scrambled.eggs:443` route to the correct container, even as it migrates between providers. Integrates with the Moshpit browser extension resolver from PRD `0001`, and is the ingress that the legacy-DNS gateway in PRD `0004` (Moshpit Bridge) proxies into for extension-less visitors.
